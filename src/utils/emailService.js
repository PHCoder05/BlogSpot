// Email Service - Direct SMTP without third-party dependencies
// Uses Simple SMTP for direct email delivery

import SimpleSmtpService from './simpleSmtpService';
...
export class EmailService {
  ...
  static async sendBlogNotification(to, blogData) {
    try {
      ...
      // Priority 1: Simple SMTP (Direct connection, no third-party)
      console.log('📧 Using Simple SMTP for direct email delivery...');
      const smtpResult = await SimpleSmtpService.sendEmail(to, emailContent.subject, emailContent.html);
      if (smtpResult) {
        console.log('✅ Email sent successfully via Simple SMTP');
        return true;
      }
      
      // Priority 2: Mock email sending for development (fallback)
      console.log('📧 Simple SMTP failed, using mock email service (Development mode)...');
      ...
      console.log('💡 Tip: Check your SMTP configuration for real email delivery');
      return true;
    } catch (error) {
  ...
  static async sendWelcomeEmail(to) {
    try {
      ...
      // Priority 1: Simple SMTP (Direct connection)
      console.log('📧 Using Simple SMTP for welcome email...');
      const smtpResult = await SimpleSmtpService.sendWelcomeEmail(to);
      if (smtpResult) {
        console.log('✅ Welcome email sent successfully via Simple SMTP');
        return true;
      }
      
      // Priority 2: Fallback to mock for development
      console.log('📧 Simple SMTP failed, using mock email service...');
  ...
  static async sendUnsubscribeEmail(to) {
    try {
      ...
      // Priority 1: Simple SMTP (Direct connection)
      console.log('📧 Using Simple SMTP for unsubscribe email...');
      const smtpResult = await SimpleSmtpService.sendEmail(to, '👋 You\'ve been unsubscribed', this.generateUnsubscribeEmailHTML());
      if (smtpResult) {
        console.log('✅ Unsubscribe email sent successfully via Simple SMTP');
        return true;
      }
